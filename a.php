...
<body class="bg-gray-100 font-sans">
    ...
    <script>
        ...
        function renderProducts() {
            ...
            // This is where you'd normally fetch products from the server
            // For now, we'll use sample data
            const products = [
                { name: "Burger", price: 5.99, type: "food" },
                { name: "Pizza", price: 8.99, type: "food" },
                { name: "Salad", price: 4.99, type: "food" },
                { name: "Soda", price: 1.99, type: "drink" },
                { name: "Coffee", price: 2.99, type: "drink" },
                { name: "Fries", price: 2.99, type: "food" }
            ];

            products.forEach(product => {
                const card = document.createElement("div");
                card.className = "product-card bg-white rounded-lg shadow-md p-6 flex flex-col justify-between";
                card.innerHTML = `
                    <h3 class="text-xl font-semibold mb-2">${product.name}</h3>
                    <p class="text-gray-600 mb-2">Price: $${product.price.toFixed(2)}</p>
                    <p class="text-gray-600 mb-4">Type: ${product.type.charAt(0).toUpperCase() + product.type.slice(1)}</p>
                    <button onclick="addToTable('${product.name}', ${product.price})" class="bg-green-500 text-white py-2 px-4 rounded-md hover:bg-green-600 transition duration-300 ease-in-out">Add to Invoice</button>
                `;
